...
// Note:
//
// (1) Matrix multiple computes C = A * B where A is m x k matrix, B is k x n, and C is m x n.
//
// (2) rmd_a is the remainder between the number of rows in matrices A and 8 or 4 (for algorithm 8x8 and algorithm 4x4 respectively).
//     rmd_b is the remainder between the number of rows in matrices B and 8 or 4 (for algorithm 8x8 and algorithm 4x4 respectively).
//
...
kernel void MetalGemm8x8(const device float        *A   [[ buffer(0) ]],
...
kernel void MetalGemm4x4(const device float        *A   [[ buffer(0) ]],
                         const device float        *B   [[ buffer(1) ]],
                         device float              *C   [[ buffer(2) ]],
                         constant MetalMatrixDim&  dims [[ buffer(3) ]],
                         ushort2                   gid  [[ thread_position_in_grid ]]) {
    ...
    const ushort2 gidIn = ushort2(gid.x << 2, gid.y << 2);
    ...
    if (gidIn.x + 4 > m || gidIn.y + 4 > n) {
        ushort remainderA = (gidIn.x + 4 > m) ? rmd_a: 4;
        ushort remainderB = (gidIn.y + 4 > n) ? rmd_b: 4;
        ...
        do {
            ...
            switch (remainderA) {
                case 1:
                    aCurr.x = trans_a ? a[0].x : *((device float *)a + k * 0);
                    break;
                case 2:
                    aCurr.xy = trans_a ? a[0].xy : float2(*((device float *)a + k * 0),
                                                          *((device float *)a + k * 1));
                    break;
                case 3:
                    aCurr.xyz = trans_a ? a[0].xyz : float3(*((device float *)a + k * 0),
                                                            *((device float *)a + k * 1),
                                                            *((device float *)a + k * 2));
                    break;
                case 4:
                    aCurr = trans_a ? a[0] : float4(*((device float *)a + k * 0),
                                                    *((device float *)a + k * 1),
                                                    *((device float *)a + k * 2),
                                                    *((device float *)a + k * 3));
                    break;
            ...
            switch (remainderB) {
                case 1:
                    bCurr.x = trans_b ? *((device float *)b + k * 0) : b[0].x;
                    break;
                case 2:
                    bCurr.xy = trans_b ? float2(*((device float *)b + k * 0),
                                                *((device float *)b + k * 1)) : b[0].xy;
                    break;
                case 3:
                    bCurr.xyz = trans_b ? float3(*((device float *)b + k * 0),
                                                 *((device float *)b + k * 1),
                                                 *((device float *)b + k * 2)) : b[0].xyz;
                    break;
                case 4:
                    bCurr = trans_b ? float4(*((device float *)b + k * 0),
                                             *((device float *)b + k * 1),
                                             *((device float *)b + k * 2),
                                             *((device float *)b + k * 3)) : b[0];
                    break;
        ...
        float4x4 s = float4x4(s0 ,s1 ,s2 ,s3 );
        
        device float4 *c = (device float4 *)(C + gidIn.y);
        switch (remainderB) {
            case 1:
                for (ushort i = 0;  i < remainderA; i++) {
                    c[0].x = s[i].x * alpha + c[0].x * beta;
                    c = (device float4 *)((device float *)c + n);
                }
                break;
            case 2:
                for (ushort i = 0;  i < remainderA; i++) {
                    c[0].xy = s[i].xy * alpha + c[0].xy * beta;
                    c = (device float4 *)((device float *)c + n);
                }
                break;
            case 3:
                for (ushort i = 0;  i < remainderA; i++) {
                    c[0].xyz = s[i].xyz * alpha + c[0].xyz * beta;
                    c = (device float4 *)((device float *)c + n);
                }
                break;
            case 4:
                for (ushort i = 0;  i < remainderA; i++) {
                    c[0] = s[i] * alpha + c[0] * beta;
                    c = (device float4 *)((device float *)c + n);
                }
                break;
            default:
        ...
        do {
            float4 aCurr = trans_a ? a[0] : float4(*((device float *)a + k * 0),
                                                   *((device float *)a + k * 1),
                                                   *((device float *)a + k * 2),
                                                   *((device float *)a + k * 3));
            
            float4 bCurr = trans_b ? float4(*((device float *)b + k * 0),
                                             *((device float *)b + k * 1),
                                             *((device float *)b + k * 2),
                                             *((device float *)b + k * 3)) : b[0];
        ...
        c[0] = s0 * alpha + c[0] * beta;  c = (device float4 *)((device float *)c + n);
        c[0] = s1 * alpha + c[0] * beta;  c = (device float4 *)((device float *)c + n);
        c[0] = s2 * alpha + c[0] * beta;  c = (device float4 *)((device float *)c + n);
        c[0] = s3 * alpha + c[0] * beta;
    }
}
